...
const Appdetils = () => {
    ...
    const handleInstall = () => {
        ...
        const installedApps = JSON.parse(localStorage.getItem("installedApps")) || {};
        installedApps[id] = true; // mark installed
    ...
    const handleUninstall = () => {
        ...
        const installedApps = JSON.parse(localStorage.getItem("installedApps")) || {};
        delete installedApps[id]; // remove installed flag
        localStorage.setItem("installedApps", JSON.stringify(installedApps));
    };
            ...
            <div className='max-w-[1400px] mx-auto '>
                <div className='flex flex-col md:flex-row pl-11 gap-20 mt-9 py-7 items-center shadow-2xl'>
                    ...
                    <div>
                        ...
                        <p className='text-xl text-gray-500'>Developed by <span className='text-purple-800'> {card.companyName}</span></p>
                        <div className='w-fit lg:w-[1000px] mt-4'><hr /></div>
                        ...
                        <div className={`w-55 py-4 px-4 rounded-xl font-bold transition duration-300 ${btnchange
                            ...
                            onClick={btnchange ? handleUninstall : handleInstall}
                        >
                            <button>
                                {btnchange ? "Uninstall" : `Install Now( ${card.size}) MB`}
                            </button>
                        </div>
                    </div>
                </div>

                <div className='mt-10 text-gray-400'><hr /></div>
                <div className='mt-12 '><Ratingchart rating={rating}></Ratingchart></div>
                <div className='mt-10 text-gray-400'><hr /></div>

                <div className='mt-6 mx-6 md:mx-0'>
                    <h3 className='text-2xl font-semibold'>Description</h3>
                    <div className='mb-11 pb-8 pt-4 text-gray-400 text-justify'>{card.description}</div>
                </div>
            </div>
        </div>
    );
